...
void LowEnergyCLMatching_PDHD(const char* inputFile = nullptr){
    ...
    if (!inputFile) {
        if (gROOT->GetListOfFiles()->GetEntries() > 0) {
            // Get the first argument as the input file
                         inputFile = gROOT->GetListOfFiles()->At(0)->GetName();
                                 } else {
    ...
    std::string ChannelMap_name = "ChannelMap";
    if (inputFile) {
    // load configuration file
    std::ifstream configFile(inputFile);
    if (!configFile.is_open()) {std::cerr << "Error opening config file." << std::endl;}
    std::string config_line; 
    while (std::getline(configFile, config_line)) {
        std::istringstream lineStream(config_line);
    ...
    const double z_space_window = 200; // window in Z and Y to check for isolated hits, cm
    const double y_space_window = 200;
    // matching paramters
    const double lower_t_window = 0;
    const double upper_t_window = 2500;
    const double z_window = 200.0; // acceptance window in Z around PDS hit for TPC clusters
    const double y_window = 200.0; // acceptance window in Y around PDS hit for TPC clusters
