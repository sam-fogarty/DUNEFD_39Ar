...
for (int file_index = 0; file_index < nfiles; file_index++) {
   ...
   for (int j = 0; j < pds_z_root.size(); j++){
	pdsTimestamp.push_back(pds_t_root[j]);
        pdsZ.push_back(pds_z_root[j]);
        pdsY.push_back(pds_y_root[j]);
        pdsAPA.push_back(pds_apa_root[j]);
        hit_keep_indices.push_back(j);
	}
