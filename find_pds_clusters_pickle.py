...
with open(filepaths_file, 'r') as f:
    ...
    for file in tqdm(f):
        ...
        output_filename = f"{run}_pickle/{file.split('.')[0]}_isolatedPDSHits.hdf5"
        input_filename = f"{run}_pickle/{file.split('.')[0]}.pkl"
        if not os.path.exists(input_filename):
            print(f"File '{input_filename}' does not exist, moving on.")
